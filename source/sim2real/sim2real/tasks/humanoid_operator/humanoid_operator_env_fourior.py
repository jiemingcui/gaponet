...
class HumanoidOperatorEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HumanoidOperatorEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Buffer for accumulating per-joint rewards for logging
        self.per_joint_reward_buffer = torch.zeros((self.num_envs, len(self._motion_loader.joint_sequence)), device=self.device)
        self.per_joint_reward_count = torch.zeros(self.num_envs, device=self.device)
        
        # CSV logging for per-joint rewards (only in train mode)
        self.per_joint_reward_csv_path = None
        self.per_joint_reward_csv_file = None
        self.per_joint_reward_csv_writer = None
        self.per_joint_reward_episode_count = 0
        ...
        if self.mode == "train":
            ...
            csv_dir = os.path.join(script_dir, 'logs', 'per_joint_rewards')
            os.makedirs(csv_dir, exist_ok=True)
            self.per_joint_reward_csv_path = os.path.join(csv_dir, f'per_joint_rewards_{self.run_timestamp}.csv')
            
            # Initialize CSV file with headers
            joint_names = self._motion_loader.joint_sequence
            headers = ['iteration', 'env_id', 'payload_mass'] + [f'joint_{i}_{name}' for i, name in enumerate(joint_names)]
            self.per_joint_reward_csv_file = open(self.per_joint_reward_csv_path, 'w', newline='')
            self.per_joint_reward_csv_writer = csv.writer(self.per_joint_reward_csv_file)
            self.per_joint_reward_csv_writer.writerow(headers)
            self.per_joint_reward_csv_file.flush()  # Flush headers immediately
            print(f"[INFO] Per-joint reward logging enabled. CSV file: {self.per_joint_reward_csv_path}")
            print(f"[INFO] CSV absolute path: {os.path.abspath(self.per_joint_reward_csv_path)}")
    ...
    def solve_fk(self, dof_positions: torch.Tensor):
        """Compute forward kinematics for end-effectors (hands)."""
        ret = self.robot_chain.forward_kinematics(dof_positions[:, self.joint_usd_to_fk_chain])

        # Choose end-effector link names based on robot type
        # For original H1 robot, links are named 'L_hand_base_link' / 'R_hand_base_link'
        # For Fourior robot, hand links are 'left_hand_roll_link' / 'right_hand_roll_link'
        if hasattr(self.cfg, "robot_name") and "fourior" in self.cfg.robot_name.lower():
            eff_names = ["left_hand_roll_link", "right_hand_roll_link"]
        else:
        ...
        for eff in eff_names:
            if eff not in ret:
                raise KeyError(f"End-effector link '{eff}' not found in kinematic chain. Available links: {list(ret.keys())}")
    ...
    def _pre_physics_step(self, actions: torch.Tensor): 
        self.actions = actions.clone()    # shape: (num_envs, 31) - 所有 31 个关节的 delta action
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # Log per-joint rewards for completed episodes before reset
        if len(env_ids) > 0 and self.per_joint_reward_count[env_ids].sum() > 0:
            if "log" not in self.extras:
                self.extras["log"] = {}
            
            # Calculate mean per-joint reward for completed episodes
            joint_names = self._motion_loader.joint_sequence
            valid_mask = self.per_joint_reward_count[env_ids] > 0
            valid_env_ids = env_ids[valid_mask]
            
            if valid_mask.any():
                # Write to CSV file (only in train mode)
                if self.mode == "train" and self.per_joint_reward_csv_writer is not None:
                    for env_id in valid_env_ids.tolist():
                        # Calculate mean per-joint reward for this episode
                        episode_avg_rewards = (self.per_joint_reward_buffer[env_id] / self.per_joint_reward_count[env_id]).cpu().numpy()
                        
                        # Get payload mass for this environment
                        payload_mass = self.wrist_payload_mass[env_id].item()
                        
                        # Prepare row: episode, env_id, payload_mass, joint_0_reward, joint_1_reward, ...
                        row = [
                            self.per_joint_reward_episode_count,
                            int(env_id),
                            f"{payload_mass:.6f}"
                        ] + [f"{reward:.6f}" for reward in episode_avg_rewards]
                        
                        self.per_joint_reward_csv_writer.writerow(row)
                        self.per_joint_reward_episode_count += 1
                    
                    # Flush to ensure data is written
                    self.per_joint_reward_csv_file.flush()
                
                # Also log to extras for tensorboard/wandb
                for i, joint_name in enumerate(joint_names):
                    if i < self.per_joint_reward_buffer.shape[1]:
                        # Get episode averages for each completed environment
                        episode_avg_rewards = self.per_joint_reward_buffer[valid_env_ids, i] / self.per_joint_reward_count[valid_env_ids]
                        # Store mean across all completed episodes in this reset batch
                        self.extras["log"][f"reward_per_joint/{joint_name}"] = episode_avg_rewards.mean().item()
            
            # Reset buffers for completed episodes
            self.per_joint_reward_buffer[env_ids] = 0
            self.per_joint_reward_count[env_ids] = 0

        self.robot.reset(env_ids)  # type: ignore
        super()._reset_idx(env_ids) # type: ignore

        self.robot.write_joint_state_to_sim(self._motion_loader.dof_positions[motion_indices, time_indices], self._motion_loader.dof_velocities[motion_indices, time_indices], None, env_ids)   # type: ignore shape: (num_actions,)
        if self._motion_loader.hand_marker is not None:
    ...
    def _compute_metrics(self, play_runs_by_motion, save_path: str | None = None, combined_csv_path: str | None = None):
        ...
        def run_per_joint_upper_body_area(seq, key):
            if 'sim_joint_pos_diff' not in seq[-1]:
                return np.zeros(self.num_actions)  # Return zeros for all joints if no sim data
        ...
        def print_per_joint_table(title, bins, joint_names=None):
            ...
            if joint_names is None:
                joint_names = [f"Joint_{i}" for i in range(self.num_actions)]  # Default to all delta action joints
        ...
        def append_per_joint_csv(section_title, bins, joint_names=None):
            ...
            if joint_names is None:
                ...
                if actual_joint_count is None:
                    actual_joint_count = self.num_actions  # fallback to default (all delta action joints)
    ...
    def close(self):
        ...
        if self.per_joint_reward_csv_file is not None:
            self.per_joint_reward_csv_file.close()
            print(f"[INFO] Per-joint reward CSV file closed: {self.per_joint_reward_csv_path}")
        ...
        if self.mode == "play":
            ...
            for i in range(self._motion_loader.motion_num):
                ...
                if self._motion_loader.sim_dof_positions is not None and 'sim_joint_pos' in raw[-1]:
                    ...
                    # Format: (time_length, 1, 4, num_actions) - delta_joint_pos, sim_joint_pos (placeholder), joint_target_pos, real_joint_pos
    ...
    def step_operator(self, delta_action: torch.Tensor,
        ...
        # delta_action 现在有 31 维，对应所有 31 个关节
        # 对所有 joint_sequence_index 应用 delta_action
        ...
        # Log per-joint rewards for completed episodes before reset
        done_env_ids = self._ALL_INDICES[dones]
        if len(done_env_ids) > 0 and self.per_joint_reward_count[done_env_ids].sum() > 0:
            if "log" not in self.extras:
                self.extras["log"] = {}
            ...
            joint_names = self._motion_loader.joint_sequence
            logged_joints = []
            per_joint_rewards_dict = {}  # Store rewards for CSV writing
            
            for i, joint_name in enumerate(joint_names):
                if i < self.per_joint_reward_buffer.shape[1]:
                    # Calculate mean per episode for each completed environment
                    valid_mask = self.per_joint_reward_count[done_env_ids] > 0
                    if valid_mask.any():
                        # Get episode averages for each completed environment
                        episode_avg_rewards = self.per_joint_reward_buffer[done_env_ids[valid_mask], i] / self.per_joint_reward_count[done_env_ids[valid_mask]]
                        # Store mean across all completed episodes in this reset batch
                        # This gives the average reward per joint across all environments that just completed
                        reward_value = episode_avg_rewards.mean().item()
                        self.extras["log"][f"reward_per_joint/{joint_name}"] = reward_value
                        per_joint_rewards_dict[joint_name] = reward_value
                        logged_joints.append(f"{joint_name}: {reward_value:.4f}")
            
            # Note: CSV writing is now done in runner's log function to sync with terminal output
            # Debug: print first few logged joints to verify code is executing
            if len(logged_joints) > 0:
                print(f"[DEBUG] Logged {len(logged_joints)} per-joint rewards. First 3: {logged_joints[:3]}")
            
            # Reset buffers for completed episodes
            self.per_joint_reward_buffer[done_env_ids] = 0
            self.per_joint_reward_count[done_env_ids] = 0

        self.sample_all_environments(env_ids=done_env_ids)

        self.last_delta_action[:] = self.delta_action
        
        # Prepare return info dict - include log info if available
        info_dict = {'episode': {
        ...
        # Add per-joint rewards to episode dict (so they get logged by runner)
        # The runner checks "episode" first, so we need to put log info inside episode dict
        if "log" in self.extras and len(self.extras["log"]) > 0:
            # Merge log info into episode dict so runner will collect it
            info_dict["episode"].update(self.extras["log"])
            # Also keep log key for compatibility
            info_dict["log"] = self.extras["log"].copy()
        
        return None, rewards, dones, info_dict
