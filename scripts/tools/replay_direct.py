...
def compare_states(state_from_dataset, runtime_state, runtime_env_index) -> (bool, str): # type: ignore
...
def main():
    ...
    assert isinstance(env, DirectRLEnv), "Environment must be a DirectRLEnv"
    assert hasattr(env, "reset_to"), "Environment must have a reset_to method"
